...
func (s *Storage) SaveWithGroups(instances []*Instance, groupTree *GroupTree) error {
	...
	for i, inst := range instances {
		...
		toolData := statedb.MarshalToolData(
			...
			string(inst.CodexDetectionState), inst.CodexDetectionReason, inst.CodexDetectionAttempts,
...
func (s *Storage) LoadLite() ([]*InstanceData, []*GroupData, error) {
	...
	for i, r := range dbRows {
		claudeSID, claudeAt,
			...
			codexSID, codexAt, codexState, codexReason, codexAttempts,
		...
		instances[i] = &InstanceData{
			...
			CodexDetectionState:    CodexDetectionState(codexState),
			CodexDetectionReason:   codexReason,
			CodexDetectionAttempts: codexAttempts,
...
func (s *Storage) convertToInstances(data *StorageData) ([]*Instance, []*GroupData, error) {
	...
	for i, instData := range data.Instances {
		...
		inst := &Instance{
			...
			CodexDetectionState:    instData.CodexDetectionState,
			CodexDetectionReason:   instData.CodexDetectionReason,
			CodexDetectionAttempts: instData.CodexDetectionAttempts,
